...
int getDirExGBK(const char *comppath, LPEXTENTIONS pExt)
{
	...
	for(i = 0; i < count; i ++)
	{
			_memset(&files[nfiles], 0x00, sizeof(SceIoDirent));//���files[nfiles]
			...
			else
			{
				...
				_strcpy(szGbkFiles[nfiles],pszGbk);//���������ļ�����������������
			}
			_strcpy(files[nfiles].d_name,info[i].filename);
			...
			if( nfiles<MAX_ENTRY)
			{
				...
				if(files[nfiles].d_stat.st_attr == FAT_FILEATTR_DIRECTORY)//����Ŀ¼
				{
				...
				sortfiles[nfiles] = files + nfiles;//�����ļ�
				if (pExt) //�Ƿ��ж���չ��
				{
